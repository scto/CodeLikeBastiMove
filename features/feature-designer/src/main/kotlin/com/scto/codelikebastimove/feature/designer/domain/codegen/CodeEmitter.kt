...
class CodeEmitter {
    ...
    fun generateCode(
        ...
        val composableBody = blockTree.rootBlocks.joinToString("\n") { block ->
            generateBlockCode(block, indent = 2)
        }
        
        val functionName = blockTree.name
            .replace(" ", "")
        ...
        val fullCode = if (config.includeImports) {
            buildString {
    ...
    private fun generateBlockCode(block: Block, indent: Int): String {
        ...
        return when (block.type) {
            ...
            BlockType.FAB, BlockType.EXTENDED_FAB -> generateFabCode(block, indent)
    ...
    private fun generateColumnCode(block: Block, indent: Int): String {
        ...
        val childrenCode = block.children.joinToString("\n") { child ->
    ...
    private fun generateRowCode(block: Block, indent: Int): String {
        val indentStr = "    ".repeat(indent)
        val horizontalArrangement = block.properties["horizontalArrangement"]?.value as? String ?: "Start"
    ...
    private fun generateNavigationBarCode(block: Block, indent: Int): String {
        ...
        return buildString {
            ...
            appendLine("${indentStr}        onClick = { /* TODO */ },")
            appendLine("${indentStr}        icon = { Icon(Icons.Default.Home, contentDescription = null) },")
    ...
    private fun generateCustomComponentCode(block: Block, indent: Int): String {
        ...
        return buildString {
            appendLine("${indentStr}$componentName()")
        }
    }
        ...
        if (braceCount != 0) {
            errors.add(
                ValidationError(
                    line = null,
                    column = null,
                    message = "Mismatched curly braces: ${if (braceCount > 0) "missing $braceCount closing" else "extra ${-braceCount} closing"}"
                )
            )
        }
        ...
        if (parenCount != 0) {
            errors.add(
                ValidationError(
                    line = null,
                    column = null,
                    message = "Mismatched parentheses: ${if (parenCount > 0) "missing $parenCount closing" else "extra ${-parenCount} closing"}"
                )
            )
        }
        ...
        if (!code.contains("@Composable")) {
            errors.add(
                ValidationError(
                    line = 1,
                    column = null,
                    message = "Missing @Composable annotation"
                )
            )
        }
