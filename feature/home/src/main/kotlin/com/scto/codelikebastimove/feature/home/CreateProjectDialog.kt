...
fun CreateProjectDialog(
  ...
  AlertDialog(
    ...
    title = {
      Text(text = "Neues Projekt erstellen", style = MaterialTheme.typography.headlineSmall)
    },
    text = {
          ...
          Card(
            ...
            colors =
              CardDefaults.cardColors(
                containerColor =
                  if (selectedTemplate == template) MaterialTheme.colorScheme.primaryContainer
        ...
        Row(
          ...
          ProjectLanguage.entries.forEach { language ->
            Row(
              ...
              Text(text = language.displayName, style = MaterialTheme.typography.bodyMedium)
            }
          }
        }
        ...
        Row(
          ...
          GradleLanguage.entries.forEach { language ->
            Row(
              ...
              RadioButton(selected = selectedGradleLanguage == language, onClick = null)
              Spacer(modifier = Modifier.width(8.dp))
              Text(text = language.displayName, style = MaterialTheme.typography.bodyMedium)
            }
          }
      ...
      Button(
        onClick = {
          ...
          scope.launch {
            ...
            val outputPath =
              if (rootDirectory.isNotBlank()) rootDirectory
              else context.filesDir.absolutePath + "/projects"
            ...
            result.fold(
              onSuccess = { project -> onProjectCreated(project) },
              onFailure = { error ->
    ...
    dismissButton = { TextButton(onClick = onDismiss, enabled = !isCreating) { Text("Abbrechen") } },
  )
}
