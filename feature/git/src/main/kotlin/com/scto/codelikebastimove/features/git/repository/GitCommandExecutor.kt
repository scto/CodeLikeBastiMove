...
class GitCommandExecutor(private var workingDirectory: String = "") {
  ...
  suspend fun execute(vararg args: String): CommandResult =
      ...
      try {
        if (!checkGitAvailability()) {
          return@withContext CommandResult(
            ...
            stdout = "",
            stderr =
              "Git binary not available. This feature requires git to be installed or bundled with the application (e.g., via Termux packages or AndroidIDE bootstrap).",
            success = false,
          )
        }
        ...
        val processBuilder =
          ProcessBuilder(command).directory(File(workingDirectory)).redirectErrorStream(false)
    ...
    for (line in lines) {
      when {
        ...
        line.startsWith("# branch.upstream ") ->
          trackingBranch = line.removePrefix("# branch.upstream ")
        ...
        line.startsWith("1 ") || line.startsWith("2 ") -> {
          parseFileChange(line)?.let { change ->
            if (change.staged) stagedChanges.add(change) else unstagedChanges.add(change)
          }
        }
        line.startsWith("u ") -> {
  ...
  private fun parseFileChange(line: String): GitFileChange? {
    ...
    return GitFileChange(path = path, status = status, staged = stagedStatus != '.')
  }
  ...
  suspend fun getLog(branch: String?, maxCount: Int, skip: Int): GitOperationResult<GitLog> {
    ...
    val commits =
      result.stdout.lines().filter { it.isNotBlank() }.mapNotNull { parseCommitLine(it) }

    val countResult = execute("rev-list", "--count", branch ?: "HEAD")
    val totalCount = countResult.stdout.toIntOrNull() ?: commits.size

    return GitOperationResult.Success(
      GitLog(commits = commits, totalCount = totalCount, hasMore = skip + commits.size < totalCount)
    )
  }
  ...
  suspend fun getBranches(): GitOperationResult<List<GitBranch>> {
    val localResult =
      execute(
        "branch",
        "--format=%(refname:short)|%(objectname:short)|%(subject)|%(upstream:short)|%(upstream:track,nobracket)",
      )
    val remoteResult =
      execute("branch", "-r", "--format=%(refname:short)|%(objectname:short)|%(subject)")
  ...
  suspend fun getTags(): GitOperationResult<List<GitTag>> {
    val result =
      execute(
        "tag",
        "-l",
        "--format=%(refname:short)|%(objectname:short)|%(contents:subject)|%(taggername)|%(taggeremail)|%(creatordate:unix)",
      )
    if (!result.success) {
      return GitOperationResult.Error(result.stderr)
    }
