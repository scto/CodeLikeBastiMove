...
enum class ModuleTemplate(val displayName: String, val description: String) {
...
data class GradleNotation(val segments: List<String>) {
  val fullPath: String
  ...
  fun toPackageName(basePackage: String): String {
    val cleanSegments =
      segments.map { segment -> segment.replace("-", "").replace("_", "").lowercase() }
