...
class SvgToAvdConverter {
  ...
  fun convertSvgToAvd(svgContent: String, name: String = "converted_icon"): Result<AVDDocument> {
    return try {
      ...
      val width =
        extractAttribute(svgContent, "width")?.removeSuffix("px")?.toFloatOrNull() ?: viewBox.third
      val height =
        extractAttribute(svgContent, "height")?.removeSuffix("px")?.toFloatOrNull()
          ?: viewBox.fourth

      val paths = extractPaths(svgContent)
      val groups = extractGroups(svgContent)

      val rootGroup = VectorGroup(id = "root", name = name, paths = paths, groups = groups)
  ...
  fun convertAvdToXml(document: AVDDocument): String {
    return buildString {
      ...
      document.tint?.let { tint -> appendLine("""    android:tint="${colorToHex(tint)}"""") }
      appendLine(""">""")
  ...
  fun convertAvdToComposeImageVector(
    document: AVDDocument,
    packageName: String = "com.example",
  ): String {
    val iconName =
      document.name.split("_").joinToString("") { it.replaceFirstChar { c -> c.uppercase() } }
  ...
  private fun appendGroup(builder: StringBuilder, group: VectorGroup, indent: Int) {
    ...
    for (childGroup in group.groups) {
      val hasTransforms =
        childGroup.rotation != 0f ||
          childGroup.scaleX != 1f ||
          childGroup.scaleY != 1f ||
          childGroup.translateX != 0f ||
          childGroup.translateY != 0f
  ...
  private fun appendSvgGroup(builder: StringBuilder, group: VectorGroup, indent: Int) {
    ...
    for (childGroup in group.groups) {
      ...
      if (hasTransforms) {
        ...
        if (childGroup.rotation != 0f) {
          transforms.add(
            "rotate(${childGroup.rotation}, ${childGroup.pivotX}, ${childGroup.pivotY})"
          )
        }
  ...
  private fun appendComposePathsForGroup(builder: StringBuilder, group: VectorGroup, indent: Int) {
    ...
    for (path in group.paths) {
      builder.appendLine("${indentStr}path(")
      builder.appendLine(
        "$indentStr    fill = SolidColor(Color(${colorToArgbInt(path.fillColor)})),"
      )
      if (path.strokeColor != null) {
        builder.appendLine(
          "$indentStr    stroke = SolidColor(Color(${colorToArgbInt(path.strokeColor)})),"
        )
