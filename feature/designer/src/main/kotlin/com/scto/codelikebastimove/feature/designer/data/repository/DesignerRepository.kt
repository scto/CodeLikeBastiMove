...
class DesignerRepository {
  ...
  fun updateProject(project: DesignerProject) {
    _projects.value =
      _projects.value.map {
        if (it.id == project.id) project.copy(modifiedAt = System.currentTimeMillis()) else it
      }
  }
  ...
  fun saveComponentLibrary(library: ComponentLibrary) {
    ...
    if (existing != null) {
      _componentLibraries.value =
        _componentLibraries.value.map { if (it.id == library.id) library else it }
    } else {
  ...
  fun addComponentToLibrary(libraryId: String, component: ComponentDefinition) {
    _componentLibraries.value =
      _componentLibraries.value.map { library ->
        if (library.id == libraryId) {
          library.copy(components = library.components + component)
        } else library
      }
  }
  ...
  fun removeComponentFromLibrary(libraryId: String, componentId: String) {
    _componentLibraries.value =
      _componentLibraries.value.map { library ->
        if (library.id == libraryId) {
          library.copy(components = library.components.filter { it.id != componentId })
        } else library
      }
  }
  ...
  fun saveTheme(theme: ThemeDescriptor) {
    ...
    if (existing != null) {
      _savedThemes.value = _savedThemes.value.map { if (it.name == theme.name) theme else it }
    } else {
...
class ComponentLibraryRepository {
  ...
  fun updateComponent(component: ComponentDefinition) {
    _customComponents.value =
      _customComponents.value.map {
        if (it.id == component.id) component.copy(modifiedAt = System.currentTimeMillis()) else it
      }
  }
  ...
  fun getComponentsByCategory(
    category: com.scto.codelikebastimove.feature.designer.data.model.ComponentCategory
  ...
  fun createLibrary(name: String, description: String = ""): ComponentLibrary {
    val library = ComponentLibrary(name = name, description = description)
    _userLibraries.value = _userLibraries.value + library
    return library
  }

  fun exportComponentWithPrefix(component: ComponentDefinition, prefix: String): String {
