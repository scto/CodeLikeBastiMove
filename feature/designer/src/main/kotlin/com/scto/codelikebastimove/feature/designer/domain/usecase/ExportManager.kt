...
class ExportManager(private val codeEmitter: CodeEmitter = CodeEmitter()) {
  ...
  fun exportToDirectory(
    ...
    return try {
      ...
      ExportResult(success = true, generatedCode = content, filePath = file.uri.toString())
    } catch (e: Exception) {
      ExportResult(
        success = false,
        generatedCode = content,
  ...
  fun exportTheme(themeDescriptor: ThemeDescriptor, config: ExportConfig): ExportResult {
    val themeName =
      if (config.exportThemeToRepo) {
  ...
  fun exportComponent(component: ComponentDefinition, config: ExportConfig): ExportResult {
  ...
  private fun generateThemeCode(theme: ThemeDescriptor, themeName: String): String {
    return buildString {
      ...
      theme.customColors.forEach { (key, value) -> appendLine("    $key = $value,") }
      appendLine(")")
      appendLine()
  ...
  private fun buildFileContent(packageName: String, imports: List<String>, code: String): String {
    return buildString {
      if (packageName.isNotEmpty()) {
        appendLine("package $packageName")
        appendLine()
      }
      imports.sorted().distinct().forEach { import -> appendLine("import $import") }
      appendLine()
      append(code)
    }
  }
  ...
  private fun extractPackageName(path: String): String {
    ...
    return packagePath.replace("/", ".").trimEnd('.')
  }
