...
data class SurroundPair(val open: String, val close: String)

data class FoldingConfiguration(val markers: FoldingMarkers? = null, val offSide: Boolean = false)

data class FoldingMarkers(val start: String, val end: String)
...
data class GrammarRule(val patterns: List<GrammarPattern>)

data class CaptureRule(val name: String)
...
data class HoverInfo(val contents: List<String>, val range: TextRange? = null)
...
abstract class AbstractLanguagePackPlugin : LanguagePackPlugin {
  override val description: String
    get() = "Language support for $languageName"
...
object LanguagePackExtensionPoint {
  val DESCRIPTOR =
    ExtensionPointDescriptor(
      ...
      description =
        "Language support including syntax highlighting, snippets, and language features",
      allowMultiple = true,
    )
}
