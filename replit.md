...
- **Sora Editor (`feature/soraeditor`):** Advanced code editor module leveraging Rosemoe's sora-editor and Android Tree-Sitter for syntax highlighting, intelligent editing features, and an LSP infrastructure for future language server implementations. Theme changes now properly re-apply the language so TreeSitter syntax colors refresh correctly. EditorLanguageType supports Gradle Groovy, Gradle Kotlin DSL, Properties, TOML, and all standard file types. It also includes extensive Git submodule management actions.
