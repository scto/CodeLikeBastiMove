...
- **Main Module (`feature/main`):** Central module for project operations (create, delete, open, close, clone, import) and navigation management, with persistence across app reinstalls. OpenProjectScreen scans and displays all directories from CLBMProjects root alongside stored projects.
- **Git Module (`feature/git`):** Comprehensive Git version control integration using JGit library, supporting all standard Git operations without an external binary. Includes dedicated UI for changes, history, branches, remotes, stash, tags, and settings with editable user config (name, email) and access token input. Clone progress shows percentage circle with current task name. CloneProgressInfo exposed through GitOperations interface with indeterminate/determinate state support.
...
- **Tree View (`feature/treeview`):** Enhanced file tree view with 5 view modes (Files, Android, Packages, Modules, Project), file operations, context menus, and search. FileTreeDrawer Git tab shows client-style interface with Changes/History/Branches/Remotes/Stash/Tags/Settings sections. Layout tab detects @Composable functions in the currently open file with refresh button.
- **Sora Editor (`feature/soraeditor`):** Advanced code editor module leveraging Rosemoe's sora-editor and Android Tree-Sitter for syntax highlighting, intelligent editing features, and an LSP infrastructure for future language server implementations. Theme changes now properly re-apply the language so TreeSitter syntax colors refresh correctly. TextMate mode uses TextMateColorScheme from ThemeRegistry with custom editor colors applied on top, fixing invisible text issues. EditorLanguageType supports Gradle Groovy, Gradle Kotlin DSL, Properties, TOML, and all standard file types. It also includes extensive Git submodule management actions. EditorThemes now includes 13 themes with dark/light counterpart mapping (e.g., Solarized Dark↔Light, GitHub↔GitHub Dark, Darcula↔QuietLight). CLBMApplication auto-switches editor theme when global app theme changes (dark↔light), using `getThemeForMode()` with fallback to Darcula/QuietLight. MainActivity also observes `isSystemInDarkTheme()` for reliable FOLLOW_SYSTEM mode detection.
- **Settings Module (`feature/settings`):** Redesigned following Xed-Editor preference component pattern. Features 5 reusable components in `components/` (PreferenceTemplate, PreferenceGroup, PreferenceLayout, SettingsToggle, SettingsSlider) and organized settings into sub-packages: `app/` (main hub SettingsAppScreen), `editor/` (EditorSettingsScreen with font/indentation/display/behavior/appearance groups), `about/` (AboutScreen with app info, updates, links), `debug/` (DebugSettingsScreen with logging, update intervals, onboarding reset), `general/` (GeneralSettingsScreen with theme and dynamic colors). All screens use ViewModel internally for self-contained state management.
