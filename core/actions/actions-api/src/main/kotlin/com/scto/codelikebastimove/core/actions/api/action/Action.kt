    ...
    fun parse(keybinding: String): Keybinding {
      ...
      val modifiers =
        parts
          .dropLast(1)
          .mapNotNull { mod ->
            KeyModifier.entries.find {
              it.symbol.equals(mod, ignoreCase = true) || it.name.equals(mod, ignoreCase = true)
            }
          }
          .toSet()
...
sealed class ActionResult {
  data class Success(val message: String? = null, val data: Any? = null) : ActionResult()

  data class Failure(val error: String, val exception: Throwable? = null) : ActionResult()

  data object Cancelled : ActionResult()

  data class Pending(val message: String = "Action pending...") : ActionResult()
