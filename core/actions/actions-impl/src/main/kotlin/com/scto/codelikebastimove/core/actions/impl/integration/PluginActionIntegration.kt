...
class ActionSystem private constructor() {
    ...
    val executor: ActionExecutor = DefaultActionExecutor(registry, eventBus as com.scto.codelikebastimove.core.actions.api.event.ActionEventEmitter)
    ...
    private fun registerDefaultKeybindings() {
        val defaultBindings = mapOf(
            ...
            BuiltinActions.FORMAT_DOCUMENT to Keybinding("F", setOf(KeyModifier.CTRL, KeyModifier.SHIFT)),
            BuiltinActions.COMMENT_LINE to Keybinding("/", setOf(KeyModifier.CTRL)),
            BuiltinActions.COMMAND_PALETTE to Keybinding("P", setOf(KeyModifier.CTRL, KeyModifier.SHIFT)),
    ...
    suspend fun executeAction(actionId: String, context: ActionContext = ActionContext.EMPTY): ActionResult {
    ...
    companion object {
        @Volatile
        private var INSTANCE: ActionSystem? = null
        
        fun getInstance(): ActionSystem {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: ActionSystem().also { INSTANCE = it }
            }
        }
    }
}
