...
class DefaultToolingBridge(
    private val config: ToolingConfig
) : ToolingBridge {
    
    override val gradleProvider: GradleOutputProvider = DefaultGradleOutputProvider(config.projectPath)
    ...
    private fun buildGradleCommand(tasks: List<String>): String {
        val wrapper = if (config.gradleWrapper.startsWith("/")) {
    ...
    companion object {
        @Volatile
        private var INSTANCE: DefaultToolingBridge? = null
        
        fun getInstance(config: ToolingConfig): DefaultToolingBridge {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: DefaultToolingBridge(config).also { INSTANCE = it }
            }
        }
    }
}
