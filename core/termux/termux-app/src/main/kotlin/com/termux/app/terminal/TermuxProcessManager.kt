...
object TermuxProcessManager {
  ...
  fun createProcess(
    ...
    return try {
      val result =
        createSubprocess(command, workingDirectory, arguments, environment, rows, columns)
      ...
      ProcessResult(pid = -1, fileDescriptor = -1, success = false, errorMessage = e.message)
    }
  }
  ...
  fun createProcessWithProcessBuilder(
    ...
    return try {
      ...
      FallbackProcessResult(process = process, success = true, errorMessage = null)
    } catch (e: IOException) {
      FallbackProcessResult(process = null, success = false, errorMessage = e.message)
    }
  }
...
class TerminalPtySession(
  ...
  fun start(): Boolean {
    val result =
      TermuxProcessManager.createProcess(
        command = command,
