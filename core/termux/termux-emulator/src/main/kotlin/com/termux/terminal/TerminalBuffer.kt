...
class TerminalBuffer(
    ...
    private var screenBuffer: Array<TerminalRow> = Array(totalRows) { TerminalRow(columns, TextStyle.NORMAL) }
    private var activeTranscriptRows: Int = 0
    private var screenFirstRow: Int = 0
    
    fun resize(newColumns: Int, newRows: Int, cursorRow: Int, cursorCol: Int, foreColor: Int, backColor: Int) {
        if (newColumns == columns && newRows == totalRows) return
        
        val newBuffer = Array(newRows) { TerminalRow(newColumns, TextStyle.encode(foreColor, backColor, 0)) }
    ...
    fun scrollDownOneLine(topMargin: Int, bottomMargin: Int, leftMargin: Int, rightMargin: Int, style: Long) {
        if (leftMargin == 0 && rightMargin == columns) {
            ...
            for (row in topMargin until bottomMargin - 1) {
                ...
                for (col in leftMargin until rightMargin) {
                    screenBuffer[destInternalRow].setChar(col, 
    ...
    fun scrollUpOneLine(topMargin: Int, bottomMargin: Int, leftMargin: Int, rightMargin: Int, style: Long) {
    ...
    fun blockClear(startCol: Int, startRow: Int, endCol: Int, endRow: Int, style: Long = TextStyle.NORMAL) {
...
class TerminalRow(private val columns: Int, style: Long) {
    ...
    fun getStyle(column: Int): Long = if (column in 0 until columns) styles[column] else TextStyle.NORMAL
