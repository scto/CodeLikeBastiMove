...
class PluginStorage(private val storageDirectory: String) {
  ...
  suspend fun getEnabledPlugins(): Set<String> =
  ...
  private suspend fun saveEnabledPlugins(pluginIds: Set<String>) =
    withContext(Dispatchers.IO) {
      try {
  ...
  suspend fun getPluginSettings(pluginId: String): Map<String, Any> =
    withContext(Dispatchers.IO) {
  ...
  suspend fun savePluginSettings(pluginId: String, settings: Map<String, Any>) =
    withContext(Dispatchers.IO) {
      try {
        val settingsFile = File(pluginSettingsDir, "$pluginId.json")
  ...
  suspend fun removePluginData(pluginId: String) =
    withContext(Dispatchers.IO) {
      val settingsFile = File(pluginSettingsDir, "$pluginId.json")
      if (settingsFile.exists()) {
        settingsFile.delete()
  ...
  suspend fun clearAllPluginData() =
    withContext(Dispatchers.IO) {
