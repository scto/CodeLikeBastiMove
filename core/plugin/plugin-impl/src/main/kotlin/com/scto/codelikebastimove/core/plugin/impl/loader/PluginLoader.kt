  ...
  suspend fun loadPlugin(
    ...
    withContext(Dispatchers.IO) {
      try {
        ...
        val manifest =
          parsePluginManifest(pluginFile)
            ?: return@withContext PluginLoadResult(
              pluginId = "unknown",
              success = false,
        ...
        val context =
          createPluginContext(manifest.id, plugin.descriptor, hostServices, securityManager)
        plugin.onLoad(context)

        loadedClassLoaders[manifest.id] = classLoader

        PluginLoadResult(pluginId = manifest.id, success = true, plugin = plugin)
      } catch (e: Exception) {
        PluginLoadResult(pluginId = "unknown", success = false, error = e)
      }
    }
  ...
  private fun parsePluginManifest(pluginFile: File): PluginManifest? {
    return try {
      JarFile(pluginFile).use { jar ->
        val manifestEntry =
          jar.getEntry("plugin.properties") ?: jar.getEntry("META-INF/plugin.properties")

        if (manifestEntry != null) {
          val properties = java.util.Properties()
          jar.getInputStream(manifestEntry).use { stream -> properties.load(stream) }
          PluginManifest(
...
private class DefaultPluginContext(
  ...
  private val logger =
    object : com.scto.codelikebastimove.core.plugin.api.context.PluginLogger {
  ...
  override fun <T : Any> getHostService(serviceClass: Class<T>): T? =
    hostServices.getService(serviceClass)
...
private class InMemoryPluginDataStore :
  com.scto.codelikebastimove.core.plugin.api.context.PluginDataStore {
