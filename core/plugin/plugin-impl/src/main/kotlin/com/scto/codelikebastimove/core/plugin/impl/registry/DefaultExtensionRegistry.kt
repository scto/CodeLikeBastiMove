...
class DefaultExtensionRegistry : ExtensionRegistry {
  ...
  override fun unregisterAllExtensions(pluginId: String) {
    extensions.values.forEach { pointExtensions -> pointExtensions.remove(pluginId) }
  }
  ...
  override fun <T : Extension> getExtensions(extensionPointId: String): List<T> {
    ...
    return pointExtensions.values.flatten().sortedByDescending { it.priority }.map { it as T }
  }

  @Suppress("UNCHECKED_CAST")
  override fun <T : Extension> getExtensions(extensionClass: KClass<T>): List<T> {
    val matchingPoint =
      extensionPoints.values.find { it.extensionClass == extensionClass } ?: return emptyList()
